  ...
  if (isLoading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-gray-950 p-6 text-center">
        ...
        <h2 className="text-xl font-black lime-accent uppercase italic tracking-tighter">Sincronizando Neurônios...</h2>
        <p className="text-gray-500 text-xs mt-2 font-bold uppercase tracking-widest">A IA está gerando sua visão</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-gray-950 p-6 text-center">
        ...
        <h2 className="text-xl font-black text-red-500 uppercase italic">Erro de Processamento</h2>
        <p className="text-gray-500 text-xs mt-2 max-w-xs">{error}</p>
        <button onClick={() => window.location.reload()} className="mt-8 px-8 py-3 bg-white text-black font-black uppercase text-[10px] rounded-xl">Tentar Novamente</button>
      </div>
    );
  }
